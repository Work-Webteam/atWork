...
class MessageDigestTest extends DigestTestBase {
  ...
  public function testInvalidEntityType() {
    ...
    $this->setExpectedException(InvalidDigestGroupingException::class, 'Tried to create a message digest without both entity_type () and entity_id (42). These either both need to be empty, or have values.');
    $this->notifierSender->send($message, $configuration, $digest_notifier->getPluginId());
  }
  ...
  public function testInvalidEntityId() {
    ...
    $this->setExpectedException(InvalidDigestGroupingException::class, 'Tried to create a message digest without both entity_type (foo) and entity_id (). These either both need to be empty, or have values.');
    $this->notifierSender->send($message, $configuration, $digest_notifier->getPluginId());
  }
